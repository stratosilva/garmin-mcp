...
(function(){
  ...
  window.demoAfterRender=function(){
    ...
    if(!window.demoStarted){window.demoStarted=true;setTimeout(()=>tour.start(0),350);}
  };
  const steps=[
    ...
    {target:'#demo-body',tag:'06 / THE CONNECTION',title:'Bring gym measurements into the picture.',body:'Entered Basic-Fit body composition measurements sit beside Garmin-connected training and recovery. Different sources become a shared context for decisions—not another dashboard to check separately.',try:'Sample measurements are fictional. Try adding one in free exploration.'},
  ...
  const tour={start(i=0){previousFocus=document.activeElement;active=true;card.hidden=false;show(i);},end(){active=false;card.hidden=true;document.querySelectorAll('.demo-highlight').forEach(e=>e.classList.remove('demo-highlight'));previousFocus?.focus({preventScroll:true});},next(){if(index===steps.length-1){this.end();document.getElementById('demo-injuries').scrollIntoView({behavior:'smooth',block:'center'});}else show(index+1);}};
