...
const records=[{date:'2026-09-15',toe:8,knee:0},{date:'2026-09-16',toe:9,knee:3}];
...
const form={elements:{namedItem:()=>date},querySelectorAll:()=>inputs};
...
ctx.fillInjuryScores();assert.equal(inputs[0].value,0);assert.equal(inputs[1].value,'');
date.value='2026-09-16';ctx.fillInjuryScores();assert.equal(inputs[0].value,3);
